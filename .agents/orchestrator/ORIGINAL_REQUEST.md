...
## Initial Request — 2026-06-30T04:33:40Z
...
4. Maintain a live implementation tracker file `implementation_tracker.md` in the working directory (C:\Users\Kingr\OneDrive\Documents\wakkawakka-local\implementation_tracker.md) covering every feature, improvement, and innovation.
5. After each batch, run type checking (npm run type-check), linting (npm run lint), and build verification (npm run build).
6. Adhere strictly to the GitHub rule: only commit and push to the wakkawakka repository (NEVER touch or commit/push to the moji repository).
7. Report progress regularly to your progress.md file so the Sentinel can scan it.
8. When all planned milestones are complete and verified, report completion to the Project Sentinel.
