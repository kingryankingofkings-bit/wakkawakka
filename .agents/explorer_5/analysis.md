...
- **Missing**:
  ...
  - **No Creator Earnings breakdown**: The "Advanced Tools" tab renders a static placeholder: _"Commerce tools are currently unavailable."_ No actual tipping, subscription, or shop sales revenues are queried or displayed.
...
_Compatibility adjustment for `Order`_:
...
function EarningsLineChart({
  ...
  return (
    <svg
      ...
      className="w-full h-36 text-primary"
    >
      {/* Grid lines */}
      <line
        x1="0"
...
Replace the static placeholder in `/analytics/page.tsx` with a live **Creator Earnings Hub** that splits income into Tabs: _Overview, Product Sales, Tips, Subscriptions_.
