...
import { Clock, Sparkles, Share2, Play, ChevronLeft, ChevronRight, X, Heart, MessageCircle } from 'lucide-react';
...
const MOCK_MEMORIES: Memory[] = [
  {
    ...
    mediaUrls: JSON.stringify(['https://picsum.photos/seed/workspace1/800/800']),
    createdAt: new Date(Date.now() - 365 * 24 * 3600 * 1000).toISOString(),
    yearsAgo: 1,
    author: { id: 'u_current', displayName: 'You', username: 'you', avatar: 'https://picsum.photos/seed/you/100/100' }
  },
  {
    id: 'm2',
...
export default function MemoriesPage() {
  ...
  const handleShareMemory = (m: Memory) => {
    toast.success(`Memory from ${m.yearsAgo} year${m.yearsAgo > 1 ? 's' : ''} ago reposted to your feed!`);
  };

  const handlePrevSlide = () => {
    if (activeSlideshowIndex === null) return;
    setActiveSlideshowIndex(prev => (prev === 0 ? memories.length - 1 : (prev || 0) - 1));
  };

  const handleNextSlide = () => {
    if (activeSlideshowIndex === null) return;
    setActiveSlideshowIndex(prev => (prev === memories.length - 1 ? 0 : (prev || 0) + 1));
  };

  const activeMemory = activeSlideshowIndex !== null ? memories[activeSlideshowIndex] : null;
    ...
    <div className="min-h-screen">
      ...
      <div className="sticky top-0 z-10 bg-background/80 backdrop-blur-md border-b border-border px-4 py-3 flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-muted-foreground">Looking back at {today}</p>
        </div>
        {memories.length > 0 && (
          <Button 
            size="sm" 
      ...
      <div className="p-4 space-y-4">
        {loading && <p className="text-center text-muted-foreground py-12">Loading memories…</p>}
        {!loading && memories.length === 0 && (
          <div className="flex flex-col items-center py-20 text-center bg-card border border-border border-dashed rounded-3xl space-y-4">
            ...
            <div className="space-y-1">
              <h3 className="font-semibold text-lg text-foreground">No memories found</h3>
              <p className="text-xs text-muted-foreground max-w-sm">
                No memories from this day yet. As you post over time, they’ll resurface here.
              </p>
            </div>
          </div>
        )}
        ...
        {memories.map(m => {
          ...
          return (
            <Card key={m.id} padding="md" className="hover:shadow-md transition-shadow relative overflow-hidden">
              <div className="flex items-center justify-between mb-3 border-b border-border/50 pb-2">
                <div className="flex items-center gap-1.5 text-primary font-bold text-xs">
                  <Sparkles className="h-4 w-4" />
                  <span>{m.yearsAgo} year{m.yearsAgo > 1 ? 's' : ''} ago today</span>
                </div>
                <Button 
                  size="xs" 
                  variant="ghost" 
              ...
              <div className="flex items-center gap-3 mb-3">
                ...
                <div>
                  <p className="text-sm font-semibold text-foreground">{m.author.displayName}</p>
                  <p className="text-[10px] text-muted-foreground">{new Date(m.createdAt).toLocaleDateString()}</p>
                </div>
              </div>

              {m.content && <p className="text-sm text-foreground leading-relaxed mb-3">{m.content}</p>}

              {imgs.length > 0 && (
                <div className="grid grid-cols-2 gap-1.5 rounded-2xl overflow-hidden bg-muted/20">
                  {imgs.slice(0, 4).map((u, i) => (
                    <img key={i} src={u} alt="" className="w-full h-40 object-cover hover:scale-102 transition-transform duration-300" />
                  ))}
                </div>
              )}
            </Card>
        ...
        <div className="fixed inset-0 z-50 bg-black/98 backdrop-blur-md flex flex-col items-center justify-center p-4">
          ...
          <div className="absolute top-4 left-6 right-6 flex items-center justify-between text-white z-10">
            <div className="flex items-center gap-2">
              ...
              <span className="font-extrabold text-sm">{activeMemory.yearsAgo} Year{activeMemory.yearsAgo > 1 ? 's' : ''} Ago Today</span>
            </div>
            <button
            ...
            {memories.map((m, idx) => (
              <div
                ...
                className={cn(
                  ...
                  idx <= (activeSlideshowIndex || 0) ? "bg-primary" : "bg-white/20"
                )}
              />
            ))}
          </div>
          ...
          <div className="relative w-full max-w-lg aspect-[3/4] flex flex-col justify-between bg-card/5 border border-white/10 rounded-3xl overflow-hidden p-6 shadow-2xl">
            ...
            {parseMedia(activeMemory.mediaUrls)[0] && (
              <div 
                ...
                style={{ backgroundImage: `url(${parseMedia(activeMemory.mediaUrls)[0]})` }}
              />
            )}
            ...
            <div className="flex items-center gap-3 relative z-10">
              <Avatar src={activeMemory.author.avatar} name={activeMemory.author.displayName} size="sm" className="ring-2 ring-primary" />
              <div>
                <p className="text-sm font-bold text-white">{activeMemory.author.displayName}</p>
                <p className="text-[10px] text-white/60">{new Date(activeMemory.createdAt).toLocaleDateString()}</p>
              </div>
            </div>
