...
import { Play, Plus, Terminal, RefreshCw, Check, X, Megaphone, Activity, ShieldCheck } from 'lucide-react';

export default function DeveloperSettingsPage() {
  const [activeConsoleTab, setActiveConsoleTab] = useState<'ads' | 'webhooks'>('ads');
  ...
  const handleTriggerTest = async (subscriptionId: string) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        toast.error(data.error || 'Failed to trigger test event', { id: 'trigger-test' });
      }
    } catch (err) {
        ...
        <div className="flex bg-muted rounded-2xl p-1 border border-border">
          <button
            ...
            className={cn(
              ...
              activeConsoleTab === 'ads' ? 'bg-card text-foreground shadow-sm' : 'text-muted-foreground hover:text-foreground'
            )}
          >
          ...
          <button
            ...
            className={cn(
              'flex-1 py-2.5 text-sm font-bold rounded-xl transition-all flex items-center justify-center gap-2',
              activeConsoleTab === 'webhooks' ? 'bg-card text-foreground shadow-sm' : 'text-muted-foreground hover:text-foreground'
            )}
          >
        ...
        {isLoading ? (
          ...
          <div className="space-y-6">
            <div className="flex justify-between items-center">
              <div>
                <h2 className="font-bold text-base text-foreground">Your Ad Campaigns</h2>
                <p className="text-xs text-muted-foreground">Manage and track your active sponsored feeds</p>
              </div>
            ...
            {isCreatingAd && (
              <Card padding="md" className="border-primary/20 space-y-4">
                <div className="flex justify-between items-center border-b border-border pb-2">
                  <h3 className="font-bold text-sm text-foreground">New Ad Campaign</h3>
                  <button onClick={() => setIsCreatingAd(false)} className="p-1 hover:bg-muted rounded-full">
                    <X className="h-4 w-4" />
                  </button>
                </div>
                <form onSubmit={handleCreateAd} className="space-y-3 text-xs">
                  <div className="grid grid-cols-2 gap-3">
                    <div className="space-y-1">
                      <label className="font-semibold text-muted-foreground">Ad Title *</label>
                      <Input required placeholder="E.g., Wakka Premium Music" value={adTitle} onChange={e => setAdTitle(e.target.value)} />
                    </div>
                    <div className="space-y-1">
                      <label className="font-semibold text-muted-foreground">Target URL *</label>
                      <Input required type="url" placeholder="https://..." value={adTargetUrl} onChange={e => setAdTargetUrl(e.target.value)} />
                    </div>
                  </div>
                  <div className="space-y-1">
                    <label className="font-semibold text-muted-foreground">Ad Copy *</label>
                    <Input required placeholder="E.g., Get 50% off on all creator albums this summer!" value={adCopy} onChange={e => setAdCopy(e.target.value)} />
                  </div>
                  <div className="space-y-1">
                    <label className="font-semibold text-muted-foreground">Image URL (Optional)</label>
                    <Input placeholder="https://..." value={adImageUrl} onChange={e => setAdImageUrl(e.target.value)} />
                  </div>
                  <div className="grid grid-cols-2 gap-3">
                    <div className="space-y-1">
                      <label className="font-semibold text-muted-foreground">Total Budget ($) *</label>
                      <Input required type="number" step="0.01" placeholder="100.00" value={adBudget} onChange={e => setAdBudget(e.target.value)} />
                    </div>
                    <div className="space-y-1">
                      <label className="font-semibold text-muted-foreground">Bid Per Click ($) *</label>
                      <Input required type="number" step="0.01" placeholder="0.25" value={adBidAmount} onChange={e => setAdBidAmount(e.target.value)} />
                    </div>
                  </div>

                  {/* Demographic Targeting */}
                  <div className="border-t border-border pt-2 mt-2 space-y-2">
                    <h4 className="font-bold text-muted-foreground text-[10px] uppercase">Demographic Targeting</h4>
                    <div className="grid grid-cols-2 gap-3">
                      <div className="space-y-1">
                        <label className="font-semibold text-muted-foreground">Min Age</label>
                        <Input type="number" placeholder="18" value={adAgeMin} onChange={e => setAdAgeMin(e.target.value)} />
                      </div>
                      <div className="space-y-1">
                        <label className="font-semibold text-muted-foreground">Max Age</label>
                        <Input type="number" placeholder="45" value={adAgeMax} onChange={e => setAdAgeMax(e.target.value)} />
                      </div>
                    </div>
                    <div className="grid grid-cols-2 gap-3">
                      <div className="space-y-1">
                      ...
                      <div className="space-y-1">
                        <label className="font-semibold text-muted-foreground">Target Location</label>
                        <Input placeholder="E.g., London" value={adLocation} onChange={e => setAdLocation(e.target.value)} />
                      </div>
                    </div>
                  </div>

                  <div className="flex justify-end gap-2 pt-2">
                    <Button variant="ghost" onClick={() => setIsCreatingAd(false)}>Cancel</Button>
                    ...
                    <Card key={ad.id} padding="md" className="space-y-3">
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <p className="text-xs text-muted-foreground truncate max-w-sm">{ad.copy}</p>
                        </div>
                        <span className={cn(
                          'px-2 py-0.5 rounded-full text-[10px] font-bold uppercase',
                          ad.isActive ? 'bg-green-500/10 text-green-600' : 'bg-red-500/10 text-red-500'
                        )}>
                      ...
                      <div className="grid grid-cols-4 gap-2 text-center border-t border-border pt-3">
                        <div>
                          ...
                          <p className="text-sm font-bold text-foreground">{ad.impressions}</p>
                        </div>
                        <div>
                        ...
                        <div>
                          ...
                          <p className="text-sm font-bold text-foreground">{formatCurrency(ad.spent)} / {formatCurrency(ad.budget)}</p>
                        </div>
                      </div>

                      {/* Targeting config */}
                      <div className="text-[10px] text-muted-foreground bg-muted/40 p-2 rounded-xl">
                        <span className="font-semibold text-foreground">Targeting:</span> Age {ad.ageMin || '13'}-{ad.ageMax || '100'} · Gender: {ad.gender || 'All'} · Location: {ad.location || 'Global'}
                      </div>
                    </Card>
                  );
                })}
          ...
          <div className="space-y-6">
            <div className="flex justify-between items-center">
              <div>
                <h2 className="font-bold text-base text-foreground">Webhook Subscriptions</h2>
                <p className="text-xs text-muted-foreground">Configure HTTP callbacks for real-time app events</p>
              </div>
            ...
            {isCreatingWebhook && (
              <Card padding="md" className="border-primary/20 space-y-4">
                <div className="flex justify-between items-center border-b border-border pb-2">
                  <h3 className="font-bold text-sm text-foreground">Register Webhook</h3>
                  <button onClick={() => setIsCreatingWebhook(false)} className="p-1 hover:bg-muted rounded-full">
                    <X className="h-4 w-4" />
                  </button>
                </div>
                <form onSubmit={handleCreateWebhook} className="space-y-3 text-xs">
                  <div className="space-y-1">
                    <label className="font-semibold text-muted-foreground">Payload URL *</label>
                    <Input required type="url" placeholder="https://yourserver.com/webhooks" value={webhookUrl} onChange={e => setWebhookUrl(e.target.value)} />
                  </div>
                  <div className="space-y-1">
                    <label className="font-semibold text-muted-foreground">Subscribe Events (comma separated) *</label>
                    <Input required placeholder="tip.received, post.created" value={webhookEvents} onChange={e => setWebhookEvents(e.target.value)} />
                  </div>

                  <div className="flex justify-end gap-2 pt-2">
                    <Button variant="ghost" onClick={() => setIsCreatingWebhook(false)}>Cancel</Button>
                  ...
                  <Card key={wh.id} padding="md" className="space-y-4">
                    <div className="flex items-start justify-between gap-4">
                      <div className="space-y-1 min-w-0">
                        <p className="font-bold text-sm text-foreground truncate">{wh.url}</p>
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        >
                          <Play className="h-3 w-3 fill-current text-primary" /> Test
                        </button>
                      </div>
                    </div>
                    ...
                    <div className="border-t border-border pt-3">
                      <div className="flex items-center gap-1.5 mb-2 text-xs font-bold text-muted-foreground uppercase tracking-wider">
                        <Activity className="h-3.5 w-3.5" /> Delivery Logs ({wh.deliveryLogs?.length || 0})
                      </div>
                      {wh.deliveryLogs?.length === 0 ? (
                        <p className="text-[10px] text-muted-foreground italic">No events delivered yet.</p>
                      ) : (
                        <div className="space-y-1.5 max-h-40 overflow-y-auto divide-y divide-border/50 pr-1">
                          {wh.deliveryLogs.map((log: any) => (
                            <div key={log.id} className="pt-1.5 first:pt-0 flex flex-col gap-1 text-[10px]">
                              <div className="flex items-center justify-between">
                                <div className="flex items-center gap-1.5">
                                  {log.success ? (
                                    <span className="flex items-center gap-0.5 text-green-500 font-bold">
                                      <Check className="h-3 w-3" /> {log.statusCode || 200}
                                    </span>
                                  ) : (
                                    <span className="flex items-center gap-0.5 text-red-500 font-bold">
                                      <X className="h-3 w-3" /> {log.statusCode || 'FAIL'}
                                    </span>
                                  )}
                              ...
                              <div className="bg-muted/30 p-1.5 rounded-lg font-mono text-[9px] text-muted-foreground break-all max-h-16 overflow-y-auto">
                                <p className="font-semibold text-foreground">Payload:</p>
                                {log.payload}
                                {log.responseBody && (
                                  <>
                                    <p className="font-semibold text-foreground mt-1">Response:</p>
                                    {log.responseBody}
                                  </>
                                )}
                              </div>
