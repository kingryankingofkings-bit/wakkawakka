...
export default function BountiesPage() {
  ...
  const [claims, setClaims] = useState<any[]>([
    {
      id: "1",
      bountyTitle: "Stream for 5 hours",
      user: "SpeedyGamer",
      reward: 50.0,
      date: "2026-06-29",
    },
  ...
  const handleClaim = (bounty: any) => {
    toast.success(
      `Bounty "${bounty.title}" claimed! Payout of $${bounty.rewardAmount} initiated.`,
    );
    const newClaim = {
      ...
      <div className="p-4 max-w-md mx-auto space-y-6">
          ...
          <Card padding="md" className="border-amber-500/20 space-y-4">
            ...
            <form onSubmit={handleCreateBounty} className="space-y-3 text-xs">
              <div className="space-y-1">
                ...
                <Input
                  required
                  placeholder="E.g., Host a 2 hour stream"
                  value={title}
              ...
              <div className="space-y-1">
                ...
                <Input
                  placeholder="E.g., Streaming overlay active"
                  value={description}
        ...
        {isLoading ? (
          ...
          <div className="text-center py-16 text-muted-foreground text-xs bg-card border border-border border-dashed rounded-3xl">
            No active bounties yet. Create incentives to boost user streams and
            follows!
          </div>
        ) : (
          <div className="space-y-4">
            <h2 className="font-bold text-sm text-foreground flex items-center gap-1">
              <Flame className="h-4 w-4 text-amber-500 fill-current" /> Active
                ...
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      {b.description || "Goal Incentive"}
                    </p>
                  </div>
                  <span className="text-sm font-black text-amber-500">
                    ${b.rewardAmount.toFixed(2)}
                  </span>
                </div>
        ...
        <div className="border-t border-border pt-4 space-y-3">
          ...
          <div className="space-y-2">
            {claims.map((c) => (
              <div
                key={c.id}
                className="p-3 bg-muted/40 rounded-xl flex justify-between items-center text-xs"
              >
                <div>
