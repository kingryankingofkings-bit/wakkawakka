...
import { Plus, BadgeCheck, Globe, X, ArrowLeft, TrendingUp, Users, Eye, FileText, BarChart3, Edit3 } from 'lucide-react';
...
export default function PagesPage() {
  ...
  const [activeDashboardPage, setActiveDashboardPage] = useState<PageRow | null>(null);
  const [articles, setArticles] = useState<Record<string, Article[]>>({});
  const [showCreateArticle, setShowCreateArticle] = useState(false);
  const [articleForm, setArticleForm] = useState({ title: '', content: '', coverImage: '' });
  ...
  const handlePublishArticle = () => {
    if (!activeDashboardPage || !articleForm.title || !articleForm.content) return;
    ...
    setArticles(prev => ({
      ...prev,
      [activeDashboardPage.id]: [newArticle, ...(prev[activeDashboardPage.id] || [])],
    }));
  ...
  const getPageArticles = (pageId: string): Article[] => {
    return articles[pageId] || [
      {
        id: 'mock-art-1',
        title: 'Top 5 Strategies for Organic Social Media Growth in 2026',
        content: 'Building a presence without paid advertising is harder than ever, but these core steps will help you stay ahead.',
      ...
      },
      {
        ...
        content: 'Broad marketing is dying. Brands must cultivate niche networks of super-fans to drive engagement.',
        coverImage: 'https://images.unsplash.com/photo-1556761175-b413da4baf72?auto=format&fit=crop&w=400&q=80',
        views: 840,
        likes: 54,
      ...
      <div className="min-h-screen bg-background p-6 space-y-6 max-w-5xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-border pb-5">
          ...
          <div className="flex gap-2">
            <Button size="sm" variant="outline" onClick={() => setShowCreateArticle(true)}>
        ...
        <div className="bg-card border border-border rounded-3xl overflow-hidden shadow-sm">
          <div className="h-32 bg-gradient-to-r from-primary/20 via-indigo-500/20 to-purple-500/20 relative">
            {activeDashboardPage.coverImage && (
              <img src={activeDashboardPage.coverImage} alt="" className="h-full w-full object-cover" />
            )}
          </div>
            ...
            <div className="flex items-start gap-4 -mt-10">
              ...
              <div className="pt-10 space-y-1">
                <h2 className="text-xl font-bold flex items-center gap-1.5">
                  ...
                  {activeDashboardPage.isVerified && <BadgeCheck className="h-5 w-5 text-primary" />}
                </h2>
                <p className="text-sm text-muted-foreground">{activeDashboardPage.category} · Creator Dashboard</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            { label: 'Followers', val: activeDashboardPage.followerCount, icon: Users, change: '+4.2%' },
            { label: 'Impressions', val: formatCount(totalViews * 12), icon: TrendingUp, change: '+14.6%' },
            { label: 'Reach', val: formatCount(totalViews * 8), icon: BarChart3, change: '+8.1%' },
            { label: 'Page Views', val: formatCount(totalViews * 3), icon: Eye, change: '-2.4%', negative: true },
          ].map((stat, i) => (
            <Card key={i} className="p-4 flex flex-col justify-between space-y-2">
              ...
              <div className="space-y-1">
                ...
                <div className="flex items-center gap-1.5 text-xs">
                  <span className={cn('font-semibold', stat.negative ? 'text-destructive' : 'text-success')}>
                    {stat.change}
                  </span>
        ...
        <Card className="p-6 space-y-4">
          <div className="flex items-center justify-between border-b border-border pb-3">
            <div>
              ...
              <p className="text-xs text-muted-foreground">Organic Page Reach over the past 7 days</p>
            </div>
          ...
          <div className="relative w-full h-48 pt-2">
            <svg className="w-full h-full" viewBox="0 0 600 160" preserveAspectRatio="none">
              ...
              <line x1="0" y1="80" x2="600" y2="80" stroke="var(--border)" strokeWidth="0.5" strokeDasharray="3 3" />
              <line x1="0" y1="120" x2="600" y2="120" stroke="var(--border)" strokeWidth="0.5" strokeDasharray="3 3" />
              ...
              {/* Data Points */}
              <circle cx="100" cy="100" r="4" fill="rgb(59, 130, 246)" stroke="white" strokeWidth="1.5" />
              <circle cx="300" cy="70" r="4" fill="rgb(59, 130, 246)" stroke="white" strokeWidth="1.5" />
              <circle cx="500" cy="50" r="4" fill="rgb(59, 130, 246)" stroke="white" strokeWidth="1.5" />
            </svg>
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-3">
            {pageArticles.map(art => (
              <Card key={art.id} className="p-4 hover:shadow-md transition-shadow">
                <div className="flex gap-4">
                  {art.coverImage && (
                    <div className="h-20 w-28 rounded-xl overflow-hidden shrink-0 bg-muted">
                      <img src={art.coverImage} alt="" className="h-full w-full object-cover" />
                    </div>
                  )}
                  <div className="flex-1 min-w-0 space-y-1">
                    <h4 className="font-bold text-sm text-foreground truncate">{art.title}</h4>
                    ...
                    <div className="flex items-center gap-4 text-[10px] text-muted-foreground pt-1.5 font-semibold">
                      ...
                      <span>·</span>
                      <span className="flex items-center gap-1"><TrendingUp className="h-3 w-3" /> {art.likes} likes</span>
                    </div>
                  </div>
                </div>
              </Card>
        ...
        <Modal isOpen={showCreateArticle} onClose={() => setShowCreateArticle(false)} title="Publish as Page">
          <div className="space-y-3">
            <Input
            ...
            <div className="flex justify-end gap-2 pt-2">
              <Button variant="ghost" onClick={() => setShowCreateArticle(false)}><X className="h-4 w-4 mr-1" /> Cancel</Button>
              <Button onClick={handlePublishArticle} disabled={!articleForm.title || !articleForm.content}>Publish Post</Button>
            </div>
          </div>
        </Modal>
      </div>
    ...
    <div className="min-h-screen">
      ...
      <div className="p-4 grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        {pages.map(p => {
          ...
          return (
            <Card key={p.id} padding="none" className="overflow-hidden hover:shadow-md transition-shadow">
              ...
              <div className="p-4 space-y-3">
                <div className="flex items-start gap-3 -mt-10">
                  <Avatar src={p.avatarUrl} name={p.name} size="lg" className="ring-2 ring-background bg-card" />
                ...
                <div className="flex gap-2">
                  {isMine ? (
                    ...
                    <Button size="sm" variant={following ? 'outline' : 'primary'} className="w-full"
