...
export default function MessagesPage() {
  ...
  const [messageTab, setMessageTab] = useState<'chats' | 'status' | 'channels'>('chats');
    ...
    <div className="flex flex-col h-screen">
        ...
        {['chats', 'status', 'channels'].map((tab) => (
          <button
            ...
            className={cn(
              ...
              messageTab === tab ? 'bg-card text-foreground shadow-sm' : 'text-muted-foreground hover:text-foreground'
            )}
          >
            {tab}
          </button>
      ...
      {messageTab === 'chats' && (
        <>
          ...
          <div className="px-4 py-3 border-b border-border bg-muted/20 flex gap-4 overflow-x-auto scrollbar-hide items-center">
            {/* Post note trigger */}
            <div className="flex flex-col items-center shrink-0 cursor-pointer" onClick={() => setShowNoteModal(true)}>
              ...
              <span className="text-[10px] text-muted-foreground mt-1">Leave note</span>
            </div>

            {/* Notes list */}
            {notes.map((note) => (
              <div key={note.userId} className="flex flex-col items-center shrink-0 max-w-[80px] text-center relative group">
                {/* Note Bubble */}
                ...
                <span className="text-[10px] text-muted-foreground mt-1 truncate w-full">{note.displayName}</span>
              </div>
            ))}
          </div>
                ...
                return (
                  ...
                  >
                    {conv.isGroup ? (
                      ...
                      <Avatar src={avatar} name={avatarName} size="lg" isOnline={otherMember && otherMember.id ? onlineUsers.has(otherMember.id) : false} />
                    )}
      ...
      {messageTab === 'status' && (
        <div className="flex-1 p-4 space-y-4 overflow-y-auto">
          <h2 className="font-bold text-sm text-foreground">Recent Status Updates</h2>
          <div className="space-y-3">
            <div className="flex items-center gap-3 p-3 bg-card border border-border rounded-2xl">
              <Avatar src="https://i.pravatar.cc/100?img=47" name="Alice Dev" size="md" />
              <div>
                <p className="font-semibold text-xs text-foreground">Alice Dev</p>
                <p className="text-[10px] text-muted-foreground">Updated 10m ago</p>
              </div>
            </div>
            <div className="flex items-center gap-3 p-3 bg-card border border-border rounded-2xl">
              <Avatar src="https://i.pravatar.cc/100?img=12" name="Bob Builder" size="md" />
              <div>
                <p className="font-semibold text-xs text-foreground">Bob Builder</p>
                <p className="text-[10px] text-muted-foreground">Updated 1h ago</p>
              </div>
            </div>
          </div>
          ...
          <div className="border-t border-border pt-4">
            <p className="text-[10px] text-muted-foreground font-bold uppercase tracking-wider mb-2">Sponsored Status</p>
      ...
      {messageTab === 'channels' && (
        <div className="flex-1 p-4 space-y-4 overflow-y-auto">
          <h2 className="font-bold text-sm text-foreground">Popular Channels</h2>
          <div className="space-y-3">
            <div className="p-3 bg-card border border-border rounded-2xl flex justify-between items-center">
              <div>
                <p className="font-semibold text-xs text-foreground">Wakka Tech News</p>
                <p className="text-[10px] text-muted-foreground">15,400 followers · Daily tech digests</p>
              </div>
              <Button size="xs" onClick={() => toast.success('Followed Channel!')}>Follow</Button>
            </div>
            <div className="p-3 bg-card border border-border rounded-2xl flex justify-between items-center">
              <div>
                <p className="font-semibold text-xs text-foreground">Creator Studio Hub</p>
                <p className="text-[10px] text-muted-foreground">8,200 followers · Summer albums promo</p>
              </div>
              <Button size="xs" onClick={() => toast.success('Followed Channel!')}>Follow</Button>
            </div>
          </div>
          ...
          <div className="border-t border-border pt-4">
            <p className="text-[10px] text-muted-foreground font-bold uppercase tracking-wider mb-2">Sponsored Channel Updates</p>
      ...
      {/* Note post modal */}
      <Modal isOpen={showNoteModal} onClose={() => setShowNoteModal(false)} title="Update Status Note">
        <form onSubmit={handlePostNote} className="p-4 space-y-3 text-xs">
          <p className="text-xs text-muted-foreground">Share what&apos;s on your mind. Notes disappear after 24 hours.</p>
          <Input
            required
            maxLength={60}
