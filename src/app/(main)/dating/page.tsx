...
const MOCK_EVENTS = [
  { id: '1', title: 'Tech Singles Coffee Mixer', date: '2026-07-05', time: '18:00', location: 'Seattle DownTown Coffee', type: 'Physical' },
  { id: '2', title: 'Virtual Speed Dating (Tech & Creative)', date: '2026-07-12', time: '19:30', location: 'Zoom Video Mixer', type: 'Online' },
  { id: '3', title: 'Board Games & Singles Mixer', date: '2026-07-19', time: '17:00', location: 'Metropolitan Boardroom', type: 'Physical' }
];

export default function DatingPage() {
  const [datingTab, setDatingTab] = useState<'discover' | 'profile' | 'crush' | 'events'>('discover');
      ...
      if (profJson.data) {
        ...
        try {
          ...
          if (parsedMatches.length > 0) {
            ...
            const matchesData = await Promise.all(parsedMatches.map(async (mId: string) => {
    ...
    try {
      ...
      if (res.ok) {
        if (data.match) {
          ...
          toast.success(`You matched with ${currentCard.displayName}!`, { icon: '❤️' });
        } else if (action === 'like') {
  ...
  const handleSecretCrush = async (e: React.FormEvent) => {
    ...
    try {
      // Find user
      const searchRes = await fetch(`/api/search?q=${crushUsername}`, { headers });
      const searchJson = await searchRes.json();
      const matchedUser = searchJson.data?.users?.find((u: any) => u.username.toLowerCase() === crushUsername.toLowerCase());
      ...
      if (swipeRes.ok) {
        if (swipeData.match) {
          ...
          toast.success(`Secret Crush Match! You and ${matchedUser.displayName} crushed each other!`, { icon: '❤️' });
        } else {
          toast.success(`Added @${crushUsername} to your Secret Crush list. It remains confidential.`);
        }
        setCrushUsername('');
        loadDatingData();
      }
          ...
          {['discover', 'profile', 'crush', 'events'].map((t) => (
            <button
              ...
              className={cn(
                ...
                datingTab === t ? 'bg-card text-foreground shadow-sm' : 'text-muted-foreground hover:text-foreground'
              )}
            >
              {t}
            </button>
      ...
      <div className="p-4 max-w-md mx-auto space-y-6">
        ...
        {matchTriggered && (
          <Card className="border-pink-500 bg-pink-500/10 text-center p-6 space-y-4 animate-bounce-subtle">
            ...
            <div className="space-y-1">
              <h3 className="font-extrabold text-lg text-foreground">It&apos;s a Match!</h3>
              <p className="text-xs text-muted-foreground">You and {matchTriggered.displayName} liked each other.</p>
            </div>
            <div className="flex justify-center gap-3">
              <Avatar src={useAuthStore.getState().user?.avatar} name={useAuthStore.getState().user?.displayName || ''} size="lg" />
              <Avatar src={matchTriggered.avatar} name={matchTriggered.displayName} size="lg" />
            </div>
            <Button size="sm" variant="outline" className="border-pink-500 text-pink-500 hover:bg-pink-500/20" onClick={() => setMatchTriggered(null)}>
              Keep Swiping
            </Button>
          </Card>
        )}
        ...
        {datingTab === 'discover' && (
          <div className="space-y-6">
            ...
            {currentDiscover ? (
              <Card padding="none" className="overflow-hidden border border-border relative">
                {currentDiscover.avatar ? (
                  <img src={currentDiscover.avatar} alt={currentDiscover.displayName} className="w-full aspect-square object-cover" />
                ) : (
                ...
                <div className="p-4 space-y-2 bg-gradient-to-t from-background/90 to-background/20 pt-10 absolute bottom-0 inset-x-0">
                  <div className="flex items-end justify-between">
                    <div>
                      <h3 className="font-extrabold text-lg text-foreground">{currentDiscover.displayName}</h3>
                      {currentDiscover.location && <p className="text-xs text-muted-foreground">{currentDiscover.location}</p>}
                    </div>
                  </div>
            ...
            {myMatches.length > 0 && (
              <div className="space-y-3">
                <h3 className="font-bold text-sm text-foreground">Your Matches ({myMatches.length})</h3>
                <div className="flex gap-3 overflow-x-auto py-1">
                  {myMatches.map(m => (
                    <div key={m.id} className="flex flex-col items-center shrink-0">
                      <Avatar src={m.avatar} name={m.displayName} size="md" />
                      <span className="text-[10px] text-muted-foreground mt-1 max-w-[60px] truncate">{m.displayName}</span>
                    </div>
                  ))}
                </div>
              </div>
        ...
        {datingTab === 'profile' && (
          <Card padding="md" className="space-y-4">
            <h2 className="font-bold text-base text-foreground">Dating Profile Setup</h2>
            <form onSubmit={handleUpdateProfile} className="space-y-4 text-xs">
              <div className="space-y-1">
                <label className="font-semibold text-muted-foreground">Bio</label>
                <Input placeholder="Tell us about yourself..." value={bio} onChange={e => setBio(e.target.value)} />
              </div>
              <div className="space-y-1">
                <label className="font-semibold text-muted-foreground">Looking For</label>
                <select
                  value={lookingFor}
              ...
              <div className="border-t border-border pt-3 space-y-3">
                <h4 className="font-bold text-muted-foreground text-[10px] uppercase">Dating Prompts</h4>
                {prompts.map((p, idx) => (
                  <div key={idx} className="bg-muted/40 p-2.5 rounded-xl space-y-1.5 border border-border">
              ...
              <Button type="submit" className="w-full">Save Dating Profile</Button>
            </form>
          </Card>
        )}
        ...
        {datingTab === 'crush' && (
          <div className="space-y-4">
            <Card padding="md" className="space-y-3 border-pink-500/20">
              <h2 className="font-bold text-base text-foreground">Secret Crush</h2>
              <p className="text-xs text-muted-foreground leading-relaxed">
                Add usernames of friends or other creators to your Secret Crush list. They won&apos;t know you added them, but if they add you to their list too, it will trigger a MATCH!
              </p>
            ...
            {myCrushes.length > 0 && (
              <Card padding="sm" className="bg-muted/30">
                <p className="text-xs text-muted-foreground font-semibold">
                  You have <span className="text-pink-500 font-bold">{myCrushes.length}</span> secret crushes listed.
                </p>
              </Card>
            )}
          </div>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-3">
              {MOCK_EVENTS.map(event => (
                <Card key={event.id} padding="md" className="space-y-2 border border-border">
                  ...
                  <p className="text-xs text-muted-foreground flex items-center gap-1.5">
                    <Calendar className="h-3.5 w-3.5 text-muted-foreground" /> Location: {event.location}
                  </p>
                  <Button
                    size="xs"
                    className="w-full text-pink-500 border border-pink-500/30 bg-pink-500/5 hover:bg-pink-500/10"
                    onClick={() => toast.success('Successfully RSVP\'d to mixer!')}
                  >
                    RSVP Mingle
                  </Button>
                </Card>
