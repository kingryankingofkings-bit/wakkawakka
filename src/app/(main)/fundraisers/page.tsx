...
import { Heart, Plus, ShieldCheck, CheckCircle2, DollarSign, X } from 'lucide-react';
...
export default function FundraisersPage() {
  ...
  const handleDonationSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`/api/fundraisers/${selectedFundraiser.id}/donate`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          ...(userId ? { 'x-user-id': userId } : {}),
  ...
  return (
    <div className="min-h-screen pb-12">
      ...
      <div className="p-4 max-w-md mx-auto space-y-6">
        ...
        {isCreating && (
          <Card padding="md" className="border-rose-500/20 space-y-4">
            <div className="flex justify-between items-center border-b border-border pb-2">
              <h3 className="font-bold text-sm text-foreground">Launch Charitable Fundraiser</h3>
              <button onClick={() => setIsCreating(false)} className="p-1 hover:bg-muted rounded-full">
                <X className="h-4 w-4" />
              </button>
            </div>
            <form onSubmit={handleCreateFundraiser} className="space-y-3 text-xs">
              <div className="space-y-1">
                <label className="font-semibold text-muted-foreground">Campaign Title *</label>
                <Input required placeholder="E.g., Medical relief for animal shelter" value={title} onChange={e => setTitle(e.target.value)} />
              </div>
              <div className="space-y-1">
                <label className="font-semibold text-muted-foreground">Description</label>
                <Input placeholder="Describe the cause..." value={description} onChange={e => setDescription(e.target.value)} />
              </div>
              <div className="space-y-1">
                <label className="font-semibold text-muted-foreground">Goal Amount ($) *</label>
                <Input required type="number" placeholder="5000.00" value={goalAmount} onChange={e => setGoalAmount(e.target.value)} />
              </div>
        ...
        {isLoading ? (
          ...
          <div className="text-center py-20 text-muted-foreground text-xs bg-card border border-border border-dashed rounded-3xl">
            No fundraiser campaigns published yet. Create one to support local causes!
          </div>
        ) : (
          <div className="space-y-4">
            {fundraisers.map(f => {
              const progress = Math.min(100, Math.round((f.raisedAmount / f.goalAmount) * 100)) || 0;
              return (
                <Card key={f.id} padding="md" className="space-y-3 relative overflow-hidden">
                  <div className="flex items-start justify-between">
                    <div>
                      <h4 className="font-bold text-sm text-foreground">{f.title}</h4>
                      <p className="text-xs text-muted-foreground line-clamp-2">{f.description || 'Supporting a noble cause.'}</p>
                    </div>
                  ...
                  <div className="space-y-1">
                    <div className="flex justify-between text-[10px] font-semibold text-muted-foreground">
                      ...
                      <span>Goal: {formatCurrency(f.goalAmount)} ({progress}%)</span>
                    </div>
                  ...
                  <Button
                    ...
                    onClick={() => { setSelectedFundraiser(f); setDonationStep('form'); }}
                  >
                    Donate Now
                  </Button>
                </Card>
      ...
      {/* Donation Checkout Modal */}
      {selectedFundraiser && (
        <Modal isOpen={!!selectedFundraiser} onClose={closeDonationModal} title={`Donate to: ${selectedFundraiser.title}`}>
          {donationStep === 'form' && (
            <form onSubmit={handleDonationSubmit} className="p-4 space-y-4 text-xs">
              ...
              <div className="space-y-1">
                <label className="font-semibold text-muted-foreground">Donation Amount ($) *</label>
                <Input
                  required
                  type="number"
                  placeholder="25.00"
              ...
              <div className="space-y-3">
                <div className="space-y-1">
                  <label className="font-semibold text-muted-foreground">Cardholder Name *</label>
                  <Input required placeholder="Jane Doe" value={cardName} onChange={e => setCardName(e.target.value)} />
                </div>
                <div className="space-y-1">
                  <label className="font-semibold text-muted-foreground">Card Number *</label>
                  <Input required placeholder="4242 4242 4242 4242" value={cardNumber} onChange={e => setCardNumber(e.target.value)} />
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1">
                    <label className="font-semibold text-muted-foreground">Expiry Date *</label>
                    <Input required placeholder="MM/YY" value={cardExpiry} onChange={e => setCardExpiry(e.target.value)} />
                  </div>
                  <div className="space-y-1">
                    <label className="font-semibold text-muted-foreground">CVC *</label>
                    <Input required placeholder="123" value={cardCVC} onChange={e => setCardCVC(e.target.value)} />
                  </div>
                </div>
              </div>

              <div className="flex justify-end gap-2 pt-2">
                <Button variant="ghost" onClick={closeDonationModal}>Cancel</Button>
          ...
          {donationStep === 'processing' && (
            <div className="flex flex-col items-center justify-center py-12 space-y-4">
              ...
              <p className="text-sm font-semibold">Authorizing secure donation...</p>
            </div>
          )}
          ...
          {donationStep === 'success' && (
            <div className="flex flex-col items-center justify-center py-8 space-y-4 text-center p-4">
              ...
              <div>
                <h3 className="font-bold text-base">Thank you for your donation!</h3>
                <p className="text-xs text-muted-foreground">Your contribution has been successfully credited to this campaign.</p>
              </div>
              <Button className="w-full bg-rose-500 text-white hover:bg-rose-600" onClick={closeDonationModal}>
                Close
              </Button>
            </div>
          )}
