...
export default function ModerationQueuePage() {
  ...
  const handleToggleLabel = async (
    postId: string,
    currentLabelsStr: string | undefined,
    label: string,
  ) => {
    ...
    <div className="p-6 max-w-4xl mx-auto space-y-6">
      ...
      <div className="bg-card border border-border rounded-2xl p-4 space-y-3">
        <h2 className="text-xs font-bold text-foreground uppercase tracking-wider text-muted-foreground">
          Bluesky Labeler custom definitions
        </h2>
        <div className="flex flex-wrap gap-2">
          {customLabels.map((l) => (
            <span
              key={l}
              className="px-2.5 py-1 bg-muted rounded-xl text-xs font-bold text-foreground"
            >
              {l}
            </span>
          ))}
        ...
        <form
          onSubmit={(e) => {
            ...
            if (newLabelInput && !customLabels.includes(newLabelInput)) {
              ...
              setNewLabelInput("");
              toast.success(
                `Custom moderation label "${newLabelInput}" registered!`,
              );
            }
          }}
        ...
        >
          ...
          <button
            type="submit"
            className="px-4 bg-primary text-primary-foreground text-xs font-bold rounded-xl active:scale-95 transition-all"
          >
            Register Label
          </button>
        </form>
              ...
              >
                  ...
                  <div>
                    ...
                    <h2 className="text-sm font-bold text-foreground mt-2">
                      ...
                      <span className="text-destructive capitalize">
                        {report.reason.toLowerCase().replace("_", " ")}
                      </span>
                    </h2>
                ...
                {isPending && (
                  <div className="flex flex-wrap gap-2.5 justify-end pt-2">
                    ...
                    <button
                      onClick={() =>
                        handleAction(report.id, "RESOLVED", "REMOVE_CONTENT")
                      }
                    ...
                    <button
                      onClick={() =>
                        handleAction(report.id, "RESOLVED", "BAN_USER")
                      }
