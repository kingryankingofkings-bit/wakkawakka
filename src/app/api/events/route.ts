...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (count === 0) {
      await prisma.event.create({
        data: {
          ...
          description:
            "Build awesome modules for Wakka Social Network and win prizes!",
    ...
    if (communityId) {
      ...
      if (filter === "upcoming")
        where = { ...where, startsAt: { gte: now }, visibility: "PUBLIC" };
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json(
      { error: "Failed to create event", detail: String(err) },
      { status: 500 },
    );
  }
}
