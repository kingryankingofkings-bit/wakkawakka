  ...
  try {
    ...
    orderItems.forEach(item => {
      ...
      if (dailyEarningsMap.has(dateStr)) {
        dailyEarningsMap.set(dateStr, dailyEarningsMap.get(dateStr)! + (item.quantity * item.unitPrice));
      }
    });
    singleOrders.forEach(ord => {
      const hasItems = orderItems.some(item => item.orderId === ord.id);
      if (!hasItems) {
        ...
        if (dailyEarningsMap.has(dateStr)) {
          dailyEarningsMap.set(dateStr, dailyEarningsMap.get(dateStr)! + ord.total);
        }
      }
    });
    tips.forEach(t => {
      ...
      if (dailyEarningsMap.has(dateStr)) {
        dailyEarningsMap.set(dateStr, dailyEarningsMap.get(dateStr)! + t.amount);
      }
    });
    subscriptions.forEach(s => {
      const dateStr = s.createdAt.toISOString().split('T')[0];
      if (dailyEarningsMap.has(dateStr)) {
        dailyEarningsMap.set(dateStr, dailyEarningsMap.get(dateStr)! + s.amount);
      }
    });
    ...
    const avgEngagementRate = totalViews > 0 ? parseFloat(((totalEngagement / totalViews) * 100).toFixed(2)) : 0.0;
    const topPosts = postsWithMetrics
      .sort((a, b) => b.engagementRate - a.engagementRate)
      .slice(0, 5);

    // Follower Growth & Demographic Splits
    const followerGrowth = followers.filter(f => f.createdAt >= sinceDate).length;
    ...
    followers.forEach(f => {
      ...
      const hash = f.followerId.charCodeAt(0) + (f.followerId.charCodeAt(f.followerId.length - 1) || 0);
    ...
    return NextResponse.json({ error: 'Failed to calculate creator analytics', detail: String(err) }, { status: 500 });
  }
}
