...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!question || !Array.isArray(options) || options.filter((o: string) => o?.trim()).length < 2) {
      ...
      data: {
        ...
        options: {
          create: options
            ...
            .map((text: string, i: number) => ({ text: text.trim(), position: i })),
        },
      },
    ...
    return NextResponse.json({ error: 'Failed to create poll', detail: String(err) }, { status: 500 });
  }
}
