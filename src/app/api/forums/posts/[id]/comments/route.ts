...
// GET /api/forum/posts/[id]/comments - get nested comments tree
export async function GET(
  ...
  try {
    const comments = await prisma.subforumComment.findMany({
      where: { postId },
      include: {
        author: {
          select: {
...
export async function POST(
  ...
  try {
    ...
    if (parentId) {
      const parentComment = await prisma.subforumComment.findUnique({
        where: { id: parentId },
      });
    ...
    const comment = await prisma.$transaction(async (tx) => {
      const c = await tx.subforumComment.create({
        data: {
      ...
      // Creator automatically has vote created in ForumVote
