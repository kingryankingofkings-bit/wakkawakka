  ...
  try {
    ...
    if (existing) {
      await prisma.$transaction([
        ...
        prisma.page.update({ where: { id: params.id }, data: { followerCount: { decrement: 1 } } }),
      ]);
    ...
    await prisma.$transaction([
      ...
      prisma.page.update({ where: { id: params.id }, data: { followerCount: { increment: 1 } } }),
    ]);
    return NextResponse.json({ data: { following: true } });
  } catch (err) {
    return NextResponse.json({ error: 'Failed to toggle follow', detail: String(err) }, { status: 500 });
  }
}
