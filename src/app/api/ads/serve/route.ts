...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (placementHeader) {
      availableAds = availableAds.filter(ad => {
        if (ad.placements.length > 0) {
          return ad.placements.some(p => p.type.toUpperCase() === placementHeader.toUpperCase());
        }
    ...
    const hash = user.id.charCodeAt(0) + (user.id.charCodeAt(user.id.length - 1) || 0);
    const userGender = hash % 3 === 0 ? 'Female' : hash % 3 === 1 ? 'Male' : 'Other';
    ...
    const targetedAds = availableAds.filter(ad => {
      ...
      // 2. Gender targeting
      if (ad.gender && ad.gender.toLowerCase() !== 'all' && ad.gender.toLowerCase() !== userGender.toLowerCase()) return false;

      // 3. Location targeting
      if (ad.location && ad.location.toLowerCase() !== 'all') {
        if (!user.location || !user.location.toLowerCase().includes(ad.location.toLowerCase())) {
          return false;
        }
      }
    ...
    return NextResponse.json({ error: 'Failed to serve ad', detail: String(err) }, { status: 500 });
  }
}
