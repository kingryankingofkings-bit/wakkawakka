...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!brandName || !platform || !prompt) {
      return NextResponse.json({ error: 'Missing required fields: brandName, platform, prompt' }, { status: 400 });
    }
    ...
    if (lowerPlatform === 'x' || lowerPlatform === 'twitter') {
      // Return a thread
      generatedContent = `1/ We're excited to share our latest update at ${brandName}! ${prompt} 🚀\n\n` +
      ...
      generatedContent = `✨ Big news from ${brandName}! ✨\n\n` +
        `${prompt}\n\n` +
      ...
      // Default to LinkedIn
      generatedContent = `💼 Professional Update from ${brandName} 💼\n\n` +
