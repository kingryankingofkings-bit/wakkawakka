...
export function generateUsername(displayName: string): string {
  return (
    slugify(displayName).replace(/-/g, "_") + Math.floor(Math.random() * 9999)
  );
}
...
export function isVideoFile(filename: string): boolean {
  return ["mp4", "webm", "ogg", "mov", "avi"].includes(
    getFileExtension(filename),
  );
}
...
export function computeFeedScore(post: {
  ...
  return (
    recencyScore * 0.4 + engagementScore * 0.3 + followingBonus + interestBonus
  );
}
