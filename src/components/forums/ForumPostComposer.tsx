...
export function ForumPostComposer({
  ...
  const { subforums, fetchSubforums, createPost } = useForumStore();
  
  const [subforumId, setSubforumId] = useState(defaultSubforumId || "");
  ...
  useEffect(() => {
    ...
    } else if (subforums.length > 0 && !subforumId) {
      setSubforumId(subforums[0].id);
    }
  }, [defaultSubforumId, subforums, subforumId]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!subforumId) {
      setError("Subforum selection is required");
      return;
    }
