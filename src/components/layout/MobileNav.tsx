    ...
    <nav className="fixed bottom-0 left-0 right-0 z-40 border-t border-border bg-card/95 backdrop-blur-md md:hidden safe-area-inset-bottom">
      <div className="flex items-center justify-around h-16 px-2">
        {MOBILE_NAV.map(({ href, icon: Icon, label, isCreate, badge }) => {
          const isActive = pathname === href || (href !== '/feed?create=1' && pathname.startsWith(href));
          ...
          if (isCreate) {
            return (
              <Link
                key={href}
                href={href}
                className="flex items-center justify-center h-12 w-12 rounded-2xl bg-primary text-primary-foreground shadow-lg shadow-primary/25 active:scale-95 transition-transform"
              >
